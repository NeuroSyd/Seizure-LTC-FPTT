...
def save_checkpoint(state, is_best, prefix, filename='.pth.tar'):
    print('saving at ', prefix+filename)
    torch.save(state, prefix+filename)
    if is_best:
        shutil.copyfile(prefix+filename, prefix+ '.pth.tar')

def count_parameters(model):
    return sum(p.numel() for p in model.network.parameters() if p.requires_grad)

###############################################################################################
###############################    Define Adaptive SNN layer   #########################################
###############################################################################################

b_j0 = 0.2  # neural threshold baseline
...
class SNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size,output_size, n_timesteps, P=10):
        ...
        self.input_size = [19,23,125]
        self.hidden_size = hidden_size 
        self.output_size = output_size
        self.n_timesteps = n_timesteps

        self.rnn_name = 'SNN-ltc'
        self.conv1 = SNN_Conv_cell(self.input_size,16,3,1,1,'max')
        self.conv2 = SNN_Conv_cell(self.conv1.compute_output_size(),32,3,1,1,'max')
        self.conv3 = SNN_Conv_cell(self.conv2.compute_output_size(),64,3,1,1,'max')
    ...
    def forward(self, inputs, h):
        ...
        # print (spk_conv3.shape) # (Batch, filters, classes, ) 4d
        # print (f_spike.shape) # (Batch, filters*classes*..)
        ...
        f_output = out #(batch size and classes).# print ("f_output:", f_output)
        hiddens.append(h)

        for spk in [spk_conv1,spk_conv2,spk_conv3,spk_1,spk_2]:
            #the shape of the spikes is equivalent to the model. But spk.shape is all of them.
            # self.fr = self.fr + spk.detach().cpu().numpy().mean() /5.
            self.fr = self.fr + (spk.detach() == 1).cpu().sum()
